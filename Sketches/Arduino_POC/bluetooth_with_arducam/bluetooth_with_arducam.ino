...
// REF: https://cdn.hackaday.io/files/274271173436768/avi.pdf

const int avi_header[AVIOFFSET] PROGMEM = {
	// List identifier: RIFF(4), List Size(4), List FOURCC: 'AVI '(4), List identifier: LIST(4) 
	0x52, 0x49, 0x46, 0x46, 0xD8, 0x01, 0x0E, 0x00, 0x41, 0x56, 0x49, 0x20, 0x4C, 0x49, 0x53, 0x54,

	// List Size(4), List FOURCC: hdrl(4), Chunk for Main AVI Header: avih(4), Chunk Size(4): 56
	0xD0, 0x00, 0x00, 0x00, 0x68, 0x64, 0x72, 0x6C, 0x61, 0x76, 0x69, 0x68, 0x38, 0x00, 0x00, 0x00,

	// Chunk Data: MicroSecondsPerFrame(4), MaxBytesPerSecond(4), Padding Granularity(4), Flags(4): HasIndex, MustUseIndex (The order in which the audio and video must
	// be replayed is determined by the index and may differ from the order in which those chunks occur in file), IsInterleaved (The streams are properly interleaved
	// into each other), WasCaptureFile(The file was captured. The interleave might be weird), CopyRighted, TrustCKType(Open-DML only. It indicates that the key frame
	// flags in the index are reliable)
	0xA0, 0x86, 0x01, 0x00, 0x80, 0x66, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00,

	// TotalFrames(4), InitialFrames(4), Streams(4), SuggestedBufferSize(4)
	0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

	// Width(4), Height(4), Reserved(8)
	0x40, 0x01, 0x00, 0x00, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

	// Reserved(8), List(4), List Size(4): 132, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4C, 0x49, 0x53, 0x54, 0x84, 0x00, 0x00, 0x00,

	// List FOURCC: strl(4), Chunk for stream header list element: strh(4). There is one strl list for each stream, Chunk size(4) for strh: 48, 
	// The strh chunk consist of fccType(4) that can be 'vids' or 'auds' or 'txts'
	0x73, 0x74, 0x72, 0x6C, 0x73, 0x74, 0x72, 0x68, 0x30, 0x00, 0x00, 0x00, 0x76, 0x69, 0x64, 0x73,

	// fccHandler(4) that represents FourCC of codec to be used: MJPG, Flags(4): Disabled(Stream should not be activated by default), VideoPalChanges(Stream is a video
	// stream using palettes where the palette is changing during playback), Priority(2), language(2), InitialFrames (Number of the first block of the	stream that is
	// present in the file.
	0x4D, 0x4A, 0x50, 0x47, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

	// Scale(4): 1, Rate(4): Rate/Scale=sample/second(audio) or frames/second(video), Start(4): Start time of the stream. In case of VBR audio this value indicates
	// the number of silent frames to be played before the stream starts, Length(4) Size of the stream in units as defined in rate and scale.
	0x01, 0x00, 0x00, 0x00, rate, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00, 0x00,

	// Suggested Buffer Size(4): Size of the buffer necessary to store blocks of that stream. Can be 0 (in that case application has to guess) , 
	// Quality(4): indicates the quality of the stream. Not important, SampleSize(4): number of bytes of one stream atom(that should not be split any further)
	// strf chunk(4): The stream header list element strf. The structure of strf depends on the media type. Video media uses the BITMAPINFOHEADER, whereas
	// audio streams use the WAVEFORMATEX structure.
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x73, 0x74, 0x72, 0x66,

	// Chunk Size(4): 40, Size(4): 40. Specifies the number of bytes required by the structure. (Example: 24 bit color), Width(4): 320, Height(4): 240
	...
	*/
	...
	for (frame_cnt = 0; frame_cnt < pic_num; frame_cnt++)
	{
		...
		while (length--)
		{
			...
			{
				// If the start of the capture is found.
				is_header = true;
				buf[i++] = temp_last;
				buf[i++] = temp;
			}
