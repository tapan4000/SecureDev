...
short maxDataSample = 0;
short minDataSample = 1024;
...
unsigned long sampleRate = 11103;// 22050 *.73 = 16096. Original: 22191, 11103
unsigned long bytesPerSec = 11103;
unsigned int blockAlign = 1;
unsigned int bitsPerSample = 8;
unsigned long dataSize = 0L;
unsigned long recByteCount = 0L;
unsigned long recByteSaved = 0L;
unsigned long elapsedMicrosForCardWrite = 0;
...
bool buf0ReadyToRead = false;
bool buf1ReadyToRead = false;
unsigned int bufByteCountForAdch;
byte bufWrite;
byte bufWriteForAdch;
...
void loop() {
	...
	if (isRecording && !isFirstOccurence && (millis() > (elapsedTimeInMilliSeconds + gpsIntervalInSeconds * 1000))) {
		...
		Serial.print("max:");
		Serial.println(maxDataSample);

		Serial.print("min:");
		Serial.println(minDataSample);

		Serial.print("Above:");
		Serial.println(countSampleAboveAverage);

		Serial.print("Below:");
		Serial.println(countSampleBelowAverage);
	...
	if (isRecording && buf0ReadyToRead) { // recByteCount % (BUFFERSIZE * 2) == BUFFERSIZE) {
		buf0ReadyToRead = false;
		Serial.print("-C-");
		...
		elapsedMicrosForCardWrite = micros();

		// It takes 239252 microseconds first 10 times and 38160 microseconds remaining approx 350 times for 20 second recording to write to the card if writing the buffer size of 512. It takes alternately 38184 microseconds and 260us to write 256 KB of data to the SD card.
		rec.write(buf00, BUFFERSIZE); recByteSaved += BUFFERSIZE; // save buf01 to card
		Serial.print("-CO");
		//Serial.print(micros() - elapsedMicrosForCardWrite);
		Serial.print("-");
	}
	else if (isRecording && buf1ReadyToRead) { // recByteCount % (BUFFERSIZE * 2) == 0) {
		buf1ReadyToRead = false;
		Serial.print("-D-");
		//Serial.println(recByteCount);
...
void writeWavHeader() { // write out original WAV header to file
	// Format Spec: http://www-mmsp.ece.mcgill.ca/Documents/AudioFormats/WAVE/Docs/riffmci.pdf
	recByteSaved = 0;
...
void Setup_timer2() {
	...
	//TCCR2B - [FOC2A, FOC2B, reserved, reserved, WGM22, CS22, CS21, CS20]. Setting [CS22, CS21, CS20] as 0x01 (for 1), 0x02(for 8), 0x03(for 64), 0x04(for 256), 0x05(for 1024).
	...
	//TCCR2B = _BV(CS20) | _BV(CS21); // Timer2 clock prescaler to: 64. Setting the prescaler to 64 updates the counter every 64 clock cycles. BV sets the bit value as high for specific bit. Here it sets the value to 3.
	//TCCR2A = _BV(WGM21); // Interrupt frequency =  16Mhz/(64*6 + 1) = 41558 Hz
	//OCR2A = 6;
	// The interrupt frequency should be such that the writing to SD card happens before the next interrupt occurs. At 180, the frequency is 11103 Hz, i.e. 11103 times 
	// per second or every 90 us. So, the write to SD card for 256 bytes of samples should complete within 256*90 = 23040us. For multiplier of 45, the frequency is 44321Hz i.e 44321 times per second or 
	// every 22.5 us.
				...
				OCR2A = 180; // Output Compare match register A on timer 2 set to 90 (counts from 0 to 90); It is the counter for timer 2. Corresponds to pin 11.
...
void Setup_ADC() {
	...
	// More on ADMUX: http://www.robotplatform.com/knowledge/ADC/adc_tutorial_3.html
	// ADC takes 13 clock cycles to read a new analog value. So, the maximum rate at which the sample can be read if 16MHz/13 = 1230.76 KHz considering that it would
	// take 13 clock cycles to perform one read of ADC. As, normal audio is ~40KHz, we can divide the 16MHz clock rate by 32 to get approx 38.4KHz. So, every 32th clock
	// cycle would be used to read the the ADC and it would take 13 such clock cycles to read one value. So, one ADC value would be read in 32*13 = 416 clock cycles of
	// master clock. So, the rate at which the sample would be taken is 16MHz/416 = ~38.4KHz. So, the sample is ready every 1/38.4KHz seconds = 26us
	ADMUX = 0x65; // If (65) 0110 0101, set ADC to read pin A5, ADLAR to 1 (left adjust). If (45) 0100 0101, set to read pin A5, ADLAR set to 0 (right adjust)
...
ISR(TIMER2_COMPA_vect) {
	...
	byte highValue = ADCH;
	byte adjustedValue = highValue + 40;

	if (bufByteCount == BUFFERSIZE && bufWrite == 0) {
		bufByteCount = 0;
		buf0ReadyToRead = true;
		buf1ReadyToRead = false;
		bufWrite = 1;
		Serial.print("A");
	...
	if (bufWrite == 0) { buf00[bufByteCount] = adjustedValue; }
	if (bufWrite == 1) { buf01[bufByteCount] = adjustedValue; }

	//byte lowVal = ADCL;
	////// Write out the ADCL byte.
	////recByteCount++; // increment sample counter
	////bufByteCount++;
	////if (bufByteCount == BUFFERSIZE && bufWrite == 0) {
	////	bufByteCount = 0;
	...
	// Form the 2 byte number using the ADCH and ADCL bytes that are right ordered.
	////short dataSample = ADCL | ADCH << 8;

	////if (dataSample > maxDataSample) {
	////	maxDataSample = dataSample;
	////}

	////if (dataSample > 337) {
	////	countSampleAboveAverage++;
	////}
	////else {
	////	countSampleBelowAverage++;
	////}

	////if (dataSample < minDataSample) {
	////	minDataSample = dataSample;
	////}

	////// As the data from ADCH and ADCL would be between 1 and 1024, we need to subtract 512 from that number, so that the wave oscillates between -512 and 512.
	////// However, if we are powering the Arduino with 3.3V the values would be between 0 and 675. So, we need to subtract 337.
	////dataSample = dataSample - 337;

	////// Now multiply the number by 64 to get a range between -32768 and 32767.
	////dataSample = dataSample * 32;

	////// As the short is stored as 2's complement internally in Arduino, we can simply read the high byte and assign it to first byte and read low byte and assign to byte 2.
	////byte lowByte = dataSample;
	////byte highByte = dataSample >> 8;
	...
	////if (bufWrite == 0) { buf00[bufByteCount] = highByte; }
	////if (bufWrite == 1) { buf01[bufByteCount] = highByte; }
	...
	////if (bufWrite == 0) { buf00[bufByteCount] = lowByte; }
	////if (bufWrite == 1) { buf01[bufByteCount] = lowByte; }

	//// Write out the ADCH byte after ADCL as the register values would be replaced after reading ADCH.
	//if (bufWriteForAdch == 0) { buf00[bufByteCountForAdch] = ADCH; }
	//if (bufWriteForAdch == 1) { buf01[bufByteCountForAdch] = ADCH; }
